...
export const ChallanReceipt: React.FC<ChallanReceiptProps> = ({ challan }) => {
  ...
  return (
    <div className="print-receipt-surface bg-white dark:bg-[#0F172A] rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-m3-3 print:shadow-none print:border-none transition-colors">
      {/* Top Banner */}
      <div
      ...
      <div className="p-6 sm:p-8 space-y-6">
        {/* Status Badge */}
        <div className="flex items-center justify-between p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-500/30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-mono">
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-[#1E293B] border border-slate-200 dark:border-slate-700/80 space-y-2.5">
            <div className="flex justify-between">
              <span className="text-slate-500 dark:text-slate-400">Taxpayer Name:</span>
              <span className="font-bold text-slate-800 dark:text-slate-200">{challan.taxpayerName}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500 dark:text-slate-400">PAN / GSTIN:</span>
          ...
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-[#1E293B] border border-slate-200 dark:border-slate-700/80 space-y-2.5">
            ...
            <div className="flex justify-between">
              <span className="text-slate-500 dark:text-slate-400">BSR Code / Authority:</span>
              <span className="font-bold text-slate-800 dark:text-slate-200">{challan.bsrCode}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500 dark:text-slate-400">Challan Sequence No:</span>
              <span className="font-bold text-slate-800 dark:text-slate-200">{challan.challanNo}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500 dark:text-slate-400">Tender Date & Time:</span>
              <span className="font-bold text-slate-800 dark:text-slate-200">{challan.paidOn}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500 dark:text-slate-400">Bank Ref / UTR:</span>
        ...
        <div className="p-4 rounded-2xl bg-blue-50/70 dark:bg-[#0C1E3D] border border-blue-200 dark:border-blue-500/40 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
          ...
          <button
            ...
            className="flex items-center gap-2.5 p-2 sm:px-3 sm:py-2 rounded-xl bg-white dark:bg-[#1E293B] hover:bg-blue-50 dark:hover:bg-slate-700 border border-blue-200 dark:border-slate-700 shadow-xs hover:shadow-md transition-all active:scale-95 shrink-0 cursor-pointer group text-left"
